function [pose,force] = move_particle_forces_experiments(robot,map,goal,obsDist)
    dt = robot.dt;
    obsRho = obsDist; 
    ...
    %% Combined 
    v = 1/dt*goalSpring;% + 6/dt*obsSpring;
    ...
    pose(1) = pose(1) + (vel*cos(pose(3)+normrnd(0,1e-1)))*dt;
    pose(2) = pose(2) + (vel*sin(pose(3)+normrnd(0,1e-1)))*dt;
