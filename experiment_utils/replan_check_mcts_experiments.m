...
if robots(k).replan == 1 
    ...
    continue_plan = 0; numMCTS_Iters = 1000; full_replan = 0;
    [root, best_tour, tour_states, tasksAssigned] = mcts_planning(numMCTS_Iters,...
        robots,newTaskDict,partition,opBots,i,continue_plan,full_replan,commError);
...
elseif robots(k).replan == 2 && ~isempty(opBots)%Udpate tasks to do with broken guy and connected bots
  ...
  for j = 1:length(botsLost)
      ...
      if ded
          ...
          lostLocs(j,:) = mean(robots(k).particles(whichBot,robots(k).particleGuess(whichBot)).plan(:,1:2),1);
          if any(isnan(lostLocs(j,:)))
              lostLocs(j,:) = robots(k).particles(whichBot,robots(k).particleGuess(whichBot)).pose(1:2);
          end
      end
  end
  ...
  continue_plan = 0; numMCTS_Iters = 1400; full_replan = 1;
  ...
  for r1 = 1:length(conBots)
      robots(conBots(r1)).planID = robots(conBots(r1)).planID + 1;
  end
  for r1 = 1:length(conBots)
      for r2 = 1:length(conBots)
          % if r1~=r2              
