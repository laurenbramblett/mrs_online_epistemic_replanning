function plot_plan_pretty_experiments_sideways_correct(robots_t,taskLoc,obsDist,makeVid,filename,...
...
for t = time_start:size(robots_t,2)-1
    ...
    for k = 1:numBots
        ...
        for r = 1:numBots
            for p = 1:numParticles
                ...
                if k == 1 && t >= 273 && r ==3
                    correctPose = robots(3).particles(1,1).pose;
                    plotPart(k,r,p) = scatter(correctPose(1),correctPose(2),...
    ...
    for k = 1:numBots
        ...
        if (~isempty(robots(k).commQueue) || ~isempty(robots(k).rzQueue) || ~isempty(my_setdiff(robots(k).taskQueue,robots(k).originalPlan))) && robots(k).max_v>0.1
            ...
            set(replanMarker(k),'Rotation',90);
    ...
    if t == time_start
        ...
        depotPlot = scatter(depots_all(:,1),depots_all(:,2),60,'k','>','filled');
