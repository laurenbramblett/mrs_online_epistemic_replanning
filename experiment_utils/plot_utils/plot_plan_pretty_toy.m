function plot_plan_pretty_toy(robots_t,taskLoc,obsDist,makeVid,filename,fImg_robots,no_fImg_robots,alphachannel,no_alphachannel,time_start,initPose,colorDataStore)
numParticles = size(robots_t{1}(1).particles,2);
numBots = size(robots_t{1},1);
cr = cbrewer2('qual','Set1',numBots);
figure('Position',[859.8000  236.6000  560.0000  420.0000]);
hold on
...
for t = time_start:size(robots_t,2)-1
        ...
        for j = 1:length(whichTasks)
            ...
            if whichTasks(j)>numBots
                goalScatter(whichTasks(j)) = image('CData',no_fImg_robots{colorDataStore(whichTasks(j))},...
                    ...
                    'YData',[notaskPose(j,2)-flag_offset*4 notaskPose(j,2)+flag_offset*4],...
    ...
    if sum(1-goalsObtained)>0
        ...
        for j = 1:size(taskPose,1)
            if ~isequal(robots(1).depot,taskPose(j,1:2)) && whichTasks(j)>numBots
                delete(goalScatter(whichTasks(j)));
        ...
        for r = 1:numBots
            for p = 1:numParticles
                ...
                plotPart(k,r,p) = scatter(pPlots(r,p).pose(1),pPlots(r,p).pose(2),...
                    100,cr(r,:),'MarkerEdgeColor','none','MarkerFaceColor',cr(r,:),'MarkerFaceAlpha',0.5);
                % end
            end
            delete(commLine(k,r));
    ...
    if t == time_start
        ...
        depotPlot = scatter(robots(1).depot(1),robots(2).depot(2),60,'k','v','filled');
    ...
    leg = legend([agentsPlotMark,plotPlan,startPlot,depotPlot,beliefsPlot,commPlot],plotNames,'Location','northoutside','NumColumns',1);
