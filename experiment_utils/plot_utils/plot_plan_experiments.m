function plot_plan_experiments(robots_t,taskLoc,obsDist,makeVid,filename,fImg_robots,alphachannel,time_start,initPose)
...
rbt_size = max_dir*2/100;
txt_offset = max_dir*4/50;
flag_offset = rbt_size/1.8;
            ...
            if whichTasks(j)>numBots
                goalScatter(whichTasks(j)) = scatter(taskLoc(whichTasks(j),1),...
                    taskLoc(whichTasks(j),2),'filled','x','MarkerFaceColor', ...
                                    'g','MarkerEdgeColor','k','SizeData',50);
            end
        end
    end
        ...
        for j = 1:size(taskPose,1)
            if ~isequal(robots(1).depot,taskPose(j,1:2)) %&& whichTasks(j)>numBots
                ...
                goalScatter(whichTasks(j)) = image('CData',fImg_robots{taskColors(j)},...
                    'XData',[taskPose(j,1)-flag_offset*6 taskPose(j,1)+flag_offset*6],'YData',[taskPose(j,2)-flag_offset*6 taskPose(j,2)+flag_offset*6],...
