...
for r = 1:numBots
    if k~=r
        ...
        if norm(robots(k).pose(1:2)-otherPose)<robots(k).maxRange*2-commError && bins(k)~=bins(r) && k == 1
            s = 1;
        end
        %Who isn't here and I havent planned for
        if norm(robots(k).pose(1:2)-otherPose)<robots(k).maxRange*2-commError && bins(k)~=bins(r) && ...
        ...
        elseif norm(robots(k).pose(1:2)-otherPose)<robots(k).maxRange*2-commError && bins(k)~=bins(r) && ...
