clc; clear; close
%%%%-----%%%%
addpath(genpath("experiment_utils")) 
rng(0)
...
filename          = "3bots";                      % Storage for video and data
seedDir           = "examples";
initConditionsTxt = sprintf("%s/instance%s.txt", seedDir, filename);  % Text file with initial centralized plan
allocationsTxt    = sprintf("%s/allocation%s.txt", seedDir, filename);
...
initialize_plan_mcts_experiments
...
while ((all(pdist2(allPoses(:,1:2),depot)>goalReached) || ~isempty(allPlans)) && i<numIters) || ...
  ...
  for k = 1:numBots
    ...
    [robots(k).pose_sim, cmd_vel_sim] = move_robot_forces_sim(robots(k), sim_pose, map, robots(k).goalLoc, obsDist);
    if real_robots
    ...
    if real_robots
      ...
      % robots(k).pose = robots(k).pose_sim+[rand(1,2).*0, 0];
    end
